    ...
    for menu in actual_menus:
        ...
        for dish in menu.dishes.all():
            ...
            for el in composition:
                products_volumes[el.product.name] += round(
                    float(el.volume_per_portion), 2
                )
...
def categorize_data_by_abc_xyz(data):
    ...
    values = df.loc[
        :,
        ~df.columns.isin(
            ["За цикл", "Удельный вес", "Кумулятивный удельный вес", "Категория A/B/C"]
        ),
    ]
...
def generate_abc_analysis_table():
    ...
    return full_table.to_html(
        classes="table table-striped table-bordered table-sm table_wrapper",
        justify="justify-all",
    )
