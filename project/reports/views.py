...
from main.models import NutrientNormative, list_actual_menus_for_feeding_category, count_actual_cycle_menu_days, StudentFeedingCategory
    ...
    for dish in dishes:
        ...
        for el in dish.composition:
            if get_product_cost(el.product):
                el.cost = get_product_cost(el.product) / 1000 * float(el.volume_per_portion)
            else:
                el.cost = 0
...
def render_nutrients_normative_report_first_page(request):
    ...
    return render(request, 'reports/nutrients_normative_report_first_page.html', context=context)
...
def render_nutrients_normative_report(request):
    ...
    try:
        normative = (NutrientNormative.objects.filter(student_feeding_category=student_feeding_category,
        ...
        return render(request, 'reports/partials/nutrients_normative_report.html', context=context)

    cyclic_menus = list_actual_menus_for_feeding_category(student_feeding_category)
    actual_cycle_menu_days_count = count_actual_cycle_menu_days(student_feeding_category)

    calories, proteins, fats, carbohydrates = 0, 0, 0, 0
    for menu in cyclic_menus:
        menu_calories, menu_proteins, menu_fats, menu_carbohydrates = menu.get_nutrients()
