...
from .services.warehouse_transactions import accept_to_warehouse, write_off_from_warehouse, \
    is_volume_more_than_availability


class AcceptanceForm(forms.ModelForm):
    warehouse = forms.ModelChoiceField(queryset=Warehouse.objects.all(), empty_label="Выберите склад:",
                                       widget=forms.Select(attrs={
                                           'class': 'form-select',
                                           'placeholder': 'Выберите склад',
                                       }))
    product = forms.ModelChoiceField(queryset=Product.objects.none(), empty_label="Выберите продукт:",
                                     widget=forms.Select(attrs={
                                         'class': 'form-select',
                                         'placeholder': 'Выберите продукт',
                                     }))
    volume = forms.DecimalField(max_digits=9, decimal_places=3, min_value=0.1, label="Объем",
    ...
    note = forms.CharField(widget=forms.Textarea(attrs={"class": "form-control", "placeholder": "Примечание"}), required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "contract" in self.data:
            try:
                ...
                self.fields["product"].queryset = Contract.objects.get(id=contract_id).products.all()
    ...
    def save(self, commit=True):
        ...
        accept_to_warehouse(instance.contract, instance.product, instance.volume, instance.warehouse)
        if commit:
            instance.save()
        return instance
...
class NewWriteOffForm(forms.Form):
    ...
    cause = forms.ModelChoiceField(queryset=WriteOffCause.objects.all(), label="Причина списания",
                                   empty_label="Причина списания", widget=forms.Select(attrs={
            'class': 'form-select'
        }))
    ...
    def is_valid(self):
        ...
        if is_volume_more_than_availability(self.cleaned_data['product'],
                                            self.cleaned_data['warehouse'],
                                            self.cleaned_data['volume']):
            valid = False
            self.add_error('volume',
    ...
    def save(self):
        ...
        try:
            write_off_from_warehouse(product=data['product'],
                                     volume=data['volume'],
