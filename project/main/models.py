...
class StudentFeedingCategory(models.Model):
    name = models.CharField("Название категории питающихся", max_length=100)
...
class CycleMenu(models.Model):
    ...
    meal_type = models.ForeignKey(
        MealType, on_delete=models.PROTECT, verbose_name="Приём пищи"
    )
    ...
    dishes = models.ManyToManyField(
        Dish, through="CycleMenuComposition", through_fields=("cycle_menu_day", "dish")
    )
...
class CycleMenuComposition(models.Model):
    ...
    volume_per_student = models.DecimalField(
        max_digits=5, decimal_places=2, blank=True, null=True
    )

    def get_nutrients(self):
        dish_calories, dish_proteins, dish_fats, dish_carbohydrates = (
            self.dish.get_nutrients(volume=self.volume_per_student)
        )
...
class MenuRequirement(models.Model):
    date = models.DateField(
        validators=[MinValueValidator(localdate())], verbose_name="Дата требования"
    )
    ...
    students_number = models.IntegerField(
        validators=[MinValueValidator(1)], verbose_name="Количество учеников"
    )
...
class NutrientNormative(models.Model):
    ...
    actual_since = models.DateField()
    student_feeding_category = models.ForeignKey(
        StudentFeedingCategory, on_delete=models.PROTECT
    )
