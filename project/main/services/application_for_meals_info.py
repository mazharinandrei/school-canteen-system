...
def get_total_by_student_feeding_category():
    ...
    today_applications = ApplicationForStudentMeals.objects.filter(date=localdate() + timedelta(days=1))
    ...
    for application in today_applications:
        ...
        if category in total_by_student_feeding_category_raw:
            total_by_student_feeding_category_raw[category] += application.students_number
        else:
            total_by_student_feeding_category_raw[category] = application.students_number


    for key, value in total_by_student_feeding_category_raw.items():
        total_by_student_feeding_category.append({"student_feeding_category": key,
                                                  "total": value,
...
def get_grades_without_applications(student_feeding_category):
    ...
    for grade in Grade.objects.filter(student_feeding_category=student_feeding_category):
