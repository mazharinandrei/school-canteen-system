...
def generate_technological_map_file(technological_map):
    ...
    data = {
        ...
        "technological_map_composition": TechnologicalMapComposition.objects.filter(technological_map=technological_map),
    }
...
def create_technological_map_file(ws, data, file_path):
    insert_row(ws,
               data=(data["technological_map"].dish.name,), fonts=(bold_font(),))
    i = 2
    ...
    insert_row(ws,
               row_index=i,
               data=(data["technological_map"].calories, data["technological_map"].proteins, data["technological_map"].fats, data["technological_map"].carbohydrates))

    i+=1

    insert_row(ws,
               row_index=i,
               data=("Состав ТТК на 100 г",), fonts=(bold_font(),))
    ws.merge_cells(f"A{i}:D{i}")
    i += 1

    insert_row(ws,
               row_index=i,
               data=("Продукт", "Количество, г"))
    i += 1

    for tmc in data["technological_map_composition"]:
        insert_row(ws,
                   row_index=i,
                   data=(tmc.product.name, tmc.volume))
        i += 1
