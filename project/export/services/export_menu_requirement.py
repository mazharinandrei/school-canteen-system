...
def generate_menu_requirement_file(date, student_feeding_category):
    ...
    file_name = f"export\exported_files\menu-requirement\Меню {name_of_category} {date}.xlsx"
...
def create_menu_requirement_file(ws, data, file_path):
    ...
    insert_row(ws=ws,
               row_index=i,
               data=("Необходимые продукты",),
               alignment=center_aligment())
    ws.merge_cells(f"A{i}:F{i}")
    i += 1

    insert_row(ws=ws, row_index=i,
    ...
    for el in data["product_composition"]:
        ...
        insert_row(ws=ws,
                   row_index=i,
                   data=("", el["product"].name, el['volume'], cost))
        i += 1
    return file_path
