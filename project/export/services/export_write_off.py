...
from .export_to_table.work_with_tables import insert_row, bold_font, regular_font, \
    center_aligment, with_workbook
...
def create_write_off_file(ws, date, write_offs, file_path):
    insert_row(ws=ws, row_index=1, data=("Списания со складов",), alignment=center_aligment(), fonts=(bold_font(),))
    ws.merge_cells("A1:G1")
    insert_row(ws=ws, row_index=2, data=("Дата отчёта:", date.strftime('%d.%m.%Y')), fonts=(bold_font(), regular_font()))
    insert_row(ws=ws, row_index=3, data=("Дата и время", "Склад", "Наименование продукта", "Количество, кг", "Ответственный", "Причина", "Примечание"),
    ...
    for write_off in write_offs:
        ...
        insert_row(ws,
                   row_index=i,
                   data=(write_off.datetime.strftime('%d.%m.%Y %H:%M'),
