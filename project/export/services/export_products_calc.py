...
def generate_products_calc_file(
    ...
    return create_products_calc_file(
        data=products, products_calc_info=products_calc_info, file_path=file_path
    )
...
def create_products_calc_file(ws, data, products_calc_info, file_path):
    insert_row(
        ws=ws,
        row_index=1,
        data=("Список продуктов на заказ",),
        alignment=center_aligment(),
    )
    ws.merge_cells("A1:B1")
    insert_row(
        ws=ws, row_index=2, data=("Начало периода", products_calc_info["start_date"])
    )
    insert_row(
        ws=ws, row_index=3, data=("Конец периода", products_calc_info["end_date"])
    )
    insert_row(
        ws=ws,
        row_index=4,
        data=("Категория питающихся", products_calc_info["student_feeding_category"]),
    )
    insert_row(
        ws=ws,
        row_index=5,
        data=("Количество питающихся", products_calc_info["planned_people_number"]),
    )
    insert_row(
        ws=ws,
        row_index=6,
