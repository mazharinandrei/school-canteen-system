...
def get_name_of_category(student_feeding_category):
    return (
        StudentFeedingCategory.objects.get(id=student_feeding_category).name.split()[1]
        + " лет"
    )
...
def get_menu(date, student_feeding_category):
    ...
    return MenuRequirement.objects.get(
        date=date, student_feeding_category=student_feeding_category
    )
...
def get_menu_data(date, student_feeding_category):
    ...
    for meal_type in MealType.objects.all():
        ...
        composition = MenuRequirementComposition.objects.filter(
            menu_requirement=menu, meal_type=meal_type
        )
    ...
    return {
        "date": menu.date,
        "student_feeding_category": name_of_category,
        "menu": menu_list,
    }
...
def insert_menu_data(ws, data):
    ...
    for menu_el in data["menu"]:
        ...
        insert_row(
            ...
            row_index=i,
            data=(
                f"Итого за {menu_el['meal_type'].lower()}:",
                "",
                menu_el["calories"],
                menu_el["proteins"],
                menu_el["fats"],
