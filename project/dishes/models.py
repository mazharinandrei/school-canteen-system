...
class FoodCategory(models.Model):  # TODO: добавить slug
    name = models.CharField(verbose_name='Название категории блюд',
                            max_length=100)
...
class Dish(models.Model):
    ...
    def get_actual_technological_map(self):
        try:
            return TechnologicalMap.objects.filter(dish=self.id, date__lte=localdate()).latest('date')
        except Exception:
            return None
...
class TechnologicalMap(models.Model):
    ...
    calories = models.DecimalField(max_digits=5, decimal_places=2)  # TODO: validators = []
    proteins = models.DecimalField(max_digits=5, decimal_places=2)  # TODO: validators = []
    fats = models.DecimalField(max_digits=5, decimal_places=2)  # TODO: validators = []
    carbohydrates = models.DecimalField(max_digits=5, decimal_places=2)  # TODO: validators = []
